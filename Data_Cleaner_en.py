...
def DataCleaner(status, hashtag, path):
    ...
    if status == "train":
        df = pd.read_csv(path+"/train.csv", encoding="latin-1", usecols=[0, 5], names=["label","tweet"])
        out = path+"/Cleaned_train.csv"
    elif status == "pred":
        df = pd.read_csv(path+"/"+hashtag+".csv", encoding="latin-1", usecols=[1])
    ...
    with open(path+"/tmp2.csv", newline='') as f_input, open(out, 'w', newline='') as f_output:
